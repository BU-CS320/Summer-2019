...
-- these are the same as last time, but we wrap it in a data so we can define type classes on it
...
-- we saw before that there were issues when there are multiple operators with the same precedence, this is a helper function to handle those
-- this generalizes the helper functions from before
